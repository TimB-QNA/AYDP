...
aydpSettings::aydpSettings(){
  QGridLayout *layout = new QGridLayout();

  hardwareLabel = new QLabel("Hardware");
  layout->addWidget(hardwareLabel,0,0);
  hardwareCombo = new QComboBox();
  hardwareCombo->addItem("Network");
  hardwareCombo->addItem("Serial");
  hardwareCombo->addItem("Bluetooth");
  layout->addWidget(hardwareCombo,0,1);  
...
void aydpSettings::readXML(QDomNode root){
  QDomNode node;
  QDomElement element;
...
QStringList aydpSettings::writeXML(){
  int i;
  char line[4096];
  QStringList xml;
  xml.append("  <sample>");
...
void aydpSettings::storeGUIValues(){
