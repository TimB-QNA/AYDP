...
NMEA::NMEA(QWidget *parent, bool design)
           : QWidget(parent)
{
  printf("Initialising OpenPilot::NMEA\n");
  isDesign=design;
  invalid=0;
}

void NMEA::paintEvent(QPaintEvent *event){
  if (!isDesign) return;

  QPainter painter(this);
  painter.drawRect(1,1,width()-2,height()-2);
  painter.drawText(10,20, "NMEA Parser");
}
...
int NMEA::loadFromFile(QString filename){
  ...
  if (input=='\0'){
    printf("OpenPilot::NMEA - Cannot Open File - %s\n",filename.toAscii().data());
    return 2;
  }
  for (i=0;feof(input)==0;i++){
    fgets(line,1024,input);
    if (strlen(line)>0) parseData(line);
  }
  fclose(input);
  return 0;
}
...
//    sscanf(buffer,"%*s %*f %*c %lf %c %lf %c %lf %lf",&lat,&ns,&lon,&ew,&spd,&trk);
  ...
  if (sentence.at(0)=="PGSV"){
    ...
    *$GPGSV is ignored
    */
    if (DBG) printf("OpenPilot::NMEA - Ignored PGSV Sentence\n");
    handled=true;
  }
...
/// AIS Data Extraction
  ...
  if (DBG && !handled){
    printf("OpenPilot::NMEA - Un-handled Sentence Construction\n");
    for (i=0;i<sentence.size();i++) printf("%i - %s\n",i,sentence.at(i).toAscii().data());
  }
}


void NMEA::logInput(QString input){
  QStringList dev;
  dev=input.split(" ");
  parseData(dev.at(1));
}

void NMEA::netInput(QByteArray raw){
  int i;
  QList<QByteArray> list;
  
  netBuffer.append(raw);
  list=netBuffer.split('$');
  if (list.size()<2) return;
  
  for (i=0;i<list.size()-1;i++){
//    printf("$%s\n",list.at(i).trimmed().data());
    parseData(QString('$'+list.at(i).trimmed()));
  }
  netBuffer=list.last();
}  
