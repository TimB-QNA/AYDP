...
{
  fprintf(stderr,"Creating nmeaReader\n");
...
//    fprintf(stderr,"OpenPilot::NMEA - Invalid Checksum\n");
//    fprintf(stderr,"OpenPilot::NMEA - %s\n",input.toLatin1().data());
//    return;
//  }
...
//    printf("Sats -- %i %i %lf %lf %lf\n",vesselData->fixType, vesselData->actSats, vesselData->mdop, vesselData->hdop, vesselData->vdop);
 ...
 //   printf("%s\n",input.toLatin1().data());
  ...
  if (sentence.at(0)=="GPRMC" || sentence.at(0)=="GPRM"){
    ...
    */
    if (sentence.size()<9 || sentence.at(2)!="A"){
      printf("Invalid fix\n");
      return;
    }
...
/// AIS Data Extraction
  ...
  if (!handled){
    fprintf(stderr,"Un-handled Sentence Construction\n  %s",input.toLatin1().data());
    for (i=0;i<sentence.size();i++) fprintf(stderr,"%i - %s\n",i,sentence.at(i).toLatin1().data());
  }
}
