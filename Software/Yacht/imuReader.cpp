...
{
  fprintf(stderr,"Creating imuReader\n");
...
void imuReader::run(){
  ...
  if (!spoof){
    fprintf(stderr,"Reading IMU settings\n");
    settings = new RTIMUSettings(settingsFile.toLatin1().data());
    fprintf(stderr,"Creating IMU interface\n");
    imu = RTIMU::createIMU(settings);
    fprintf(stderr,"Initialising IMU\n");
    imu->IMUInit();
