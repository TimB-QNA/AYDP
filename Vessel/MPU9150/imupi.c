...
void errorHandler(int ERC){
  if (ERC==0) printf("IMUPI_NO_ERROR\n");
  if (ERC==1) printf("IMUPI_I2C_OPEN_ERROR\n");
  if (ERC==2) printf("IMUPI_I2C_DEV_NOT_FOUND\n");
  if (ERC==3) printf("IMUPI_I2C_WRITE_ERROR\n");
  if (ERC==4) printf("IMUPI_I2C_READ_ERROR\n");
  if (ERC==5) printf("IMUPI_INIT_ERROR\n");
}
...
int imupi_init( void )  {
  ...
  ret=imupi_read( dummy_a, g, dummy_m );
  printf("g[0]=%lf\n",g[0]);
  printf("g[1]=%lf\n",g[1]);
  printf("g[2]=%lf\n",g[2]);
  if ( ret ){
    errorHandler(ret);
    return ret; //IMUPI_INIT_ERROR;
  }
  /* Calculate gyro offsets */
  
  for ( i = 0; i < IMUPI_GOFF_NB_ITER; i++ )  {
    printf("Grab gyro offsets - %i\n",i);
    if ( imupi_read( dummy_a, g, dummy_m ) ) return IMUPI_INIT_ERROR;
